...
class IPlayer:
    ...
    def testplaceFence(self, coord, direction):
        #print("player %s place %s" % (self.name, FencePlacing(coord, direction)))
        fence = self.fences[len(self.fences)-1]
        fence.test_place(coord, direction)
    def delete_Fence(self, coord, direction):
        #print("player %s place %s" % (self.name, FencePlacing(coord, direction)))
        fence = self.fences[len(self.fences)-1]      
        fence.delete_fence(coord, direction)
