...
class My_Bot1(IBot):
    def play(self, board,opp_coord) -> IAction:
        ...
        if  self.remainingFences() > 0 and len(board.storedValidFencePlacings) > 0:
            for coord_fence in board.storedValidFencePlacings:
                if(Path.ManhattanDistance(opp_coord, coord_fence.coord) < 4 or Path.ManhattanDistance(self.pawn.coord, coord_fence.coord) < 4):
                    if board.isFencePlacingBlocking(coord_fence) == 0:
                        r = (-(board.Fence_Distance(coord_fence)),coord_fence)
                        possible_fence.append(r)
                # else:
        ...
        p2 = (Path.Dijkstra(board,opp_coord,board.endPositions(0)))
        ...
        if random.randint(0,1) == 0 and self.remainingFences() > 0 and len(board.storedValidFencePlacings) > 0 and wall:
            #print(rand[0])
            if (diff - worst[0]) > (rand[0] - diff) and board.isValidFencePlacing(fence1.coord,fence1.direction) and random.randint(0,2) == 0:
                for fences in board.storedValidFencePlacings:
                    if fences.coord == fence1.coord and fences.direction == fence1.direction:
                        randomFencePlacing = fences
                        block_worst = 1
